...
import com.virjar.sekiro.utils.CommonRes;
...
//            }
            ...
            // 修改
            try {
                // 使用 BufferedReader 和 InputStreamReader 替代 IOUtils.toString()
                StringBuilder requestJSONBody = new StringBuilder();
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(httpServletRequest.getInputStream(), charset))) {
                    String line;
                    while ((line = reader.readLine()) != null) {
                        requestJSONBody.append(line);
                    }
                }
                // 解析请求的 JSON 数据
                JSONObject jsonObject = JSONObject.parseObject(requestJSONBody.toString());

                // 将解析的 JSON 数据放入 requestJson
                for (String key : jsonObject.keySet()) {
                    requestJson.put(key, jsonObject.get(key));
                }
